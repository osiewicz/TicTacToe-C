  ...
  */
  ...
  if(val >= 1 && val <= 5){
    if((fp = fopen("settings.ini","r"))){
      for(i=0;i<4;i++){
        if ((read = getline(&buffer, &len, fp)) > 0){
          ...
          if((strcmp(settings_labels[i],settings_array[i]))!=0){
            printf("Your configuration file is corrupted!'%s' is not equal to '%s'!\
			    Your config will be restored to default.\n",settings_array[i],settings_labels[i]);
            fclose(fp);
            settings(-1);
          }
          buffer = strtok(NULL,":");
      ...
      if (((strcmp(lang,languages[0]) != 0) && (strcmp(lang,languages[1]) != 0))
		      || (ai_level > 5 || ai_level < 0)){
